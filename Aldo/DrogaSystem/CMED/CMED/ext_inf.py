...
with con:
    with con.cursor() as cursor:
        ...
        cursor.execute('SELECT cmed.id, REGEXP_SUBSTR(cmed.apresenta, \'\\d.*\', (SELECT char_length(cmed.apresenta))-9) FROM cmed;')
        resposta = cursor.fetchall()
        ...
        a = 'CREATE TABLE quant (id INT REFERENCES cmed ON DELETE CASCADE ON UPDATE CASCADE,quant VARCHAR(255), PRIMARY KEY (id));'
        cursor.execute(a)

        a = 'INSERT INTO quant VALUES (%s, %s);'
        #for i in range(10):
        for i in range(len(cmed_list)):
            cursor.execute(a, resposta[i])


        cursor.execute('SELECT  cmed.id, cmed.ggerm, quant.quant FROM cmed INNER JOIN quant ON cmed.id = quant.id;')
        resposta = cursor.fetchall()
            ...
            for j in range(len(resposta[0])):
                if j == 2:
                    try:
                        ...
                        #531625403136416  
                        #print (temp[i][j])  
                elif j == 1: temp.append(int(resposta[i][j]))
        ...
        a = 'CREATE TABLE intQuantGGERM (id INT REFERENCES cmed ON DELETE CASCADE ON UPDATE CASCADE,ggerm BIGINT, quant INT, PRIMARY KEY (id));'
        cursor.execute(a)

        a = 'INSERT INTO intQuantGGERM VALUES (%s, %s, %s);'
        for i in range(len(filt_list_int)):
            cursor.execute(a, filt_list_int[i])

        a = 'SELECT cmed.id, cmed.produto, intQuantGGERM.ggerm ,intQuantGGERM.quant, cmed.pf, cmed.apresenta '
        b = 'FROM cmed INNER JOIN intQuantGGERM ON cmed.id = intQuantGGERM.id WHERE intQuantGGERM.quant >65;'
        cursor.execute(a+b)
        resposta = cursor.fetchall()
